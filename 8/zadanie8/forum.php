...
if($this->context||isset($_GET['api'])){  // --- akcje wymagajace zalogowania ---
...
// edycja postu
   if(isset($_GET['cmd']) and $_GET['cmd']=='getpost'){
      if($p=$this->post->get($_GET['postid'])){
...
// dodanie obrazka
   if(isset($_FILES['image'])&&isset($_POST['imagetitle'])){
      ...
      $p=$this->image->insert($_FILES['image']);
         
   }
   
//api postman
if(isset($_GET['cmd']) && $_GET['cmd']=="12321"){
   echo "123";
   exit;
}
...
//edycja tekstu obrazka

if(isset($_GET['cmd'])&&$_GET['cmd']=="getimg"){
   if($p=$this->image->get($_GET['imgid'])){
      echo json_encode($p, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
   }else{
      echo "<p>ERROR: Post ID: ".$_GET['postid']." not  found!</p>\n";
    }  
...
if($this->context=='posts'){
    ...
    if( isset($_POST['imagetitle']) and isset($_POST['imgid'])){
